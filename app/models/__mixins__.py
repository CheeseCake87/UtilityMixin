...
class UtilityMixin:
    ...
    @classmethod
    def _um_session_exists(cls):
        if not cls.__um_session__:
            if hasattr(cls, "query"):
                _ = cls.query
                if hasattr(_, "session") and isinstance(_.session, Session):
                    cls.__um_session__ = cls.query.session
                return

            raise AttributeError(
                dedent("""
                The class using this mixin must have access to the database session.
                Example:
    ...
    def um_create(
        ...
        if return_record:
            result = cls.__um_session__.execute(insert(cls).values(**new_values).returning(cls))
    ...
    def um_update(
        ...
        if return_record:
            q = q.returning(cls)
            return cls.__um_session__.execute(
                q.values(**updated_values)
            ).scalar_one_or_none()

        cls.__um_session__.execute(q.values(**updated_values))  # type: ignore
        cls.__um_session__.commit()
    ...
    def um_as_jsonable_dict(
        ...
        if (
            ...
            if paginate and not one_or_none:
                ...
                execute: t.Union[Result, Pagination] = cls.__um_session__.execute(execute)
        ...
        return {
            ...
            cls.__um_session__.__name__ if return_key_name is None else return_key_name: [
